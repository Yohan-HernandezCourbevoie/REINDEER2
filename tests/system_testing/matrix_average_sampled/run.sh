...
LOCAL_FOLDER="tests/system_testing/matrix_average_sampled"
INPUT_FOF="$LOCAL_FOLDER/datasets/matrix_average.fof"
...
for sampling in 0 1 2 3; do
    expected_var="EXPECTED_QUERY_OUTPUT_${sampling}"
    expected="${!expected_var}"

    RUST_LOG=warn "$reindeer2" index --input "$INPUT_FOF" -k 31 --kmer-sampling "$sampling" -o integration_test_index
    RUST_LOG=warn "$reindeer2" query --fasta "$QUERY_INPUT" --index ./integration_test_index --output-format matrix-average --output "$QUERY_OUTPUT"
