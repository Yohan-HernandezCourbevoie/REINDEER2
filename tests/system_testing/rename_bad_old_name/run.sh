...
EXPECTED_ERROR="invalid_name was not found"

cd $LOCAL_FOLDER

cargo build --quiet
RUST_LOG=warn $reindeer2 index --input $INPUT_FOF -k 31 -o integration_test_index
RD2OUTPUT=$(RUST_LOG=warn $reindeer2 rename --index integration_test_index --old-name invalid_name --new-name datasetC 2>&1)
